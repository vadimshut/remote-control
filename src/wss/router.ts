...
export const router = (): IRouter => { 
    ...
    return {
        mouse_up: (step: number) => {moove(x,y,'up', step)},
        mouse_down: (step: number) => {moove(x,y,'down', step)},
        mouse_left: (step: number) => {moove(x,y,'left', step)},
        mouse_right:(step: number) => {moove(x,y,'right', step)},
        mouse_position: () => `${x},${y}`,
